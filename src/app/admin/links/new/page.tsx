...
import { checkCodeAvailablity } from "@/services/linkService";
...
export default function CreateLinkPage() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      const data = await checkCodeAvailablity(code)
      console.log(data.code)

     // setIsAvailable(data.message);
      setChecking(false);
    }, 500); // 500ms debounce
      ...
      <Card className="w-full max-w-lg shadow-2xl rounded-2xl border border-gray-100">
        <CardContent className="p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <Button
              type="submit"
              className="w-full py-3  from-indigo-500 to-purple-600 text-white font-semibold text-lg rounded-xl hover:from-indigo-600 hover:to-purple-700 shadow-lg transition-all"
